...
func _process(delta):
	if not Global.second_boss_flag and not $Sounds/WindSound.is_playing():
		$Sounds/WindSound.play()
	if should_play_music and not current_music.is_playing():
		current_music.play()
	if bats_just_defeated:
